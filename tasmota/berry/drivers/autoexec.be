...
def sendLoraBerry()
  ...
  if telemetry_map == nil || telemetry_map.size() == 0
    print("Telemetry map is empty")
    return
  end

  var telemetry_out = {}
  for vKey: telemetry_map.keys()
    var sensorKey = telemetry_map[vKey]
    if available_sensors.find(sensorKey) != nil
      telemetry_out[vKey] = available_sensors[sensorKey]
    else
      telemetry_out[vKey] = nil
    end
  end
